...
public class RiakClient implements RiakMessageCodes {
    ...
    // private static final MapReduceResponse[] NO_MAP_REDUCE_RESPONSES = new
    // MapReduceResponse[0];

    private String node;
    private String serverVersion;
    private RiakResource resource;
    ...
    public void setClientID(String id) throws IOException {
        if ((id == null) || (id.length() < Constants.RIAK_CLIENT_ID_LENGTH)) {
            throw new IllegalArgumentException("Client ID must be at least " + Constants.RIAK_CLIENT_ID_LENGTH +
                                               " bytes long");
        }
    ...
    public String getClientID() throws IOException {
        ...
        try {
            ...
            return this.clientID.toStringUtf8();
        } finally {
            release(c);
        }
    }
    ...
    private RiakObject[] process_fetch_reply(RiakConnection c, ByteString bucket, ByteString key) throws IOException,
    ...
    public ByteString[] store(RiakObject[] values, RequestMeta meta) throws IOException {
        ...
        try {
            ...
            for (int i = 0; i < values.length; i++) {
                ...
                RPB.RpbPutReq.Builder builder = RPB.RpbPutReq.newBuilder().setBucket(value.getBucketBS()).setKey(value.getKeyBS()).setContent(value.buildContent());
        ...
        public void run() {
            ...
            try {
                for (int i = 0; i < this.vclocks.length; i++) {
                    byte[] data = this.c.receive(MSG_PutResp);
                    if (data != null) {
    ...
    public RiakObject[] store(RiakObject value, IRequestMeta meta) throws IOException {
        ...
        try {
            ...
            for (int i = 0; i < res.length; i++) {
                res[i] = new RiakObject(vclock, value.getBucketBS(), value.getKeyBS(), resp.getContents(i));
            }
    ...
    public void setBucketProperties(ByteString bucket, BucketProperties props) throws IOException {
        ...
        RPB.RpbSetBucketReq req = RPB.RpbSetBucketReq.newBuilder().setBucket(bucket).setProps(props.build()).build();
    ...
    public MapReduceResponseSource mapReduce(JSONObject obj) throws IOException {
        return mapReduce(ByteString.copyFromUtf8(obj.toString()), new RequestMeta().contentType("application/json"));
    }
    ...
    public MapReduceResponseSource mapReduce(ByteString request, IRequestMeta meta) throws IOException {
        ...
        RpbMapRedReq req = RPB.RpbMapRedReq.newBuilder().setRequest(request).setContentType(meta.getContentType()).build();
